...
export function validateSRTFile(file: MulterFile): ValidationResult {
  const errors: string[] = [];
  const warnings: string[] = [];

  // Check file size
  if (file.size > FILE_SIZE_LIMITS.MAX_SUBTITLE_SIZE) {
    errors.push(`File size ${file.size} exceeds maximum allowed size of ${FILE_SIZE_LIMITS.MAX_SUBTITLE_SIZE} bytes`);
  }

  // Check file extension
  const extension = file.originalname.split('.').pop()?.toLowerCase();
  if (!extension || extension !== 'srt') {
    errors.push(`Invalid file extension: ${extension}. Only .srt files are supported`);
  }

  // Check MIME type (allow both text/plain and application/x-subrip)
  if (!file.mimetype.includes('text') && !file.mimetype.includes('subrip')) {
    warnings.push(`Unexpected MIME type: ${file.mimetype}. Expected text/plain or application/x-subrip`);
  }

  // Basic SRT content validation
  if (file.buffer) {
    const content = file.buffer.toString('utf-8');
    if (!validateSRTContent(content)) {
      errors.push('Invalid SRT file format. Please ensure the file contains properly formatted subtitle entries');
    }
  }
...
function validateSRTContent(content: string): boolean {
  // Basic SRT format validation
  const srtPattern = /^\d+\s*\n\d{2}:\d{2}:\d{2},\d{3}\s*-->\s*\d{2}:\d{2}:\d{2},\d{3}\s*\n.+/m;
  return srtPattern.test(content.trim());
}